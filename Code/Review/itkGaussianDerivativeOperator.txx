...
::GenerateCoefficients()
{
  ...
  if ( !this->GetUseDerivativeOperator() )
    ...
    for ( int i = -(int)coeff.size() / 2; i <= (int)coeff.size() / 2; ++i )
      {
      double sum = 0.0;
      if ( m_Order % 2 == 0 ) // even
        {
        for ( j = 0, k = m_Order / 2; j < polyCoeffs.size(); j += 2, k-- )
          {
    ...
    // The input gaussian kernel needs to be padded the a clamped
    ...
    for ( unsigned int i = 0; i < 2*N - 1; ++i )
      {
      paddedCoeff[i] =  coeff.front();
      *(paddedCoeff.rbegin() + i) = coeff.back();
      }
  ...
  // derivative should accumulate to zero
  double shift = std::accumulate( coeff.begin(), coeff.end(), 0.0 ) / coeff.size();

  // enforce by shifting the values
