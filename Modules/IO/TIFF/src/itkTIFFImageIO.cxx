...
void TIFFImageIO::ReadTIFFTags()
{
  ...
  for (int i = 0; i < tagCount; ++i)
    ...
    const itkTIFFField *fld  = TIFFFieldWithTag(m_InternalImage->m_Image, tag);
    const char*         fieldName = TIFFFieldName(fld);
    ...
    const int read_count = itkTIFFFieldReadCount( fld );
    ...
    if ( itkTIFFFieldPassCount( fld ) )
      ...
      if ( itkTIFFFieldDataType( fld ) == TIFF_ASCII
        ...
        {
        const size_t dataSize = itkTIFFDataSize(itkTIFFFieldDataType( fld ));
    ...
    itkDebugMacro( << "TiffInfo tag " << fieldName << "("<< tag << "): "
                   << itkTIFFFieldDataType( fld ) << " " << value_count << " " << raw_data);


#define Encapsulate(T1,T2) if ( value_count > 1 )                       \
      {                                                                 \
      itk::Array<T1> a(value_count);                                    \
      for(unsigned int cnt = 0; cnt < value_count; ++cnt) { a[cnt] = ((const T2 *)raw_data)[cnt];} \
      EncapsulateMetaData<itk::Array<T1> >(dict, fieldName, a);         \
      } else {                                                          \
      EncapsulateMetaData<T1>(dict, fieldName,  ((const T2 *)raw_data)[0]); \
      }

    try
      {
      switch(itkTIFFFieldDataType( fld ))
        {
        case TIFF_ASCII:
          if ( value_count > 1 )
            {
            EncapsulateMetaData<std::string>(dict, fieldName, std::string( (const char *)raw_data, value_count));
            }
          else
            {
            EncapsulateMetaData<std::string>(dict, fieldName, std::string( (const char *)raw_data));
            }
          break;
        case TIFF_BYTE:
          EncapsulateMetaData<Array<char> >(dict, fieldName, Array<char>( (const char *)raw_data, value_count));
          break;
        case TIFF_SHORT:
          Encapsulate(unsigned short, uint16);
          break;
        case TIFF_LONG:
          EncapsulateMetaData<unsigned int>(dict, fieldName, ((const uint32 *)raw_data)[0]);
          break;
        case TIFF_SBYTE:
          EncapsulateMetaData<signed char>(dict, fieldName, ((const int8 *)raw_data)[0]);
          break;
        case TIFF_SSHORT:
          EncapsulateMetaData<short>(dict, fieldName, ((const int16 *)raw_data)[0]);
          break;
        case TIFF_SLONG:
          Encapsulate(int, int32);
          break;
        case TIFF_FLOAT:
          Encapsulate(float, float);
          break;
        case TIFF_DOUBLE:
          Encapsulate(double, double);
          break;
        case TIFF_IFD:
#ifdef TIFF_INT64_T // detect if libtiff4
        case TIFF_LONG8:
        ...
        case TIFF_UNDEFINED:
        default:
          itkWarningMacro( << fieldName << " has unsupported data type (" << itkTIFFFieldDataType( fld ) << ") for meta-data dictionary." )
          break;
        }
      }
    catch(...)
